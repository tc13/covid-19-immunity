...
SEIRRS_age_structure <- function(R0=2.8, latent_mean=4.5, infectious_mean=3.07, immune_mean_1=90,
  ...
  out[,,"S"] <- state[,,"S"]
  out[,,"E"] <- sum(sapply(latent_shape, FUN=function(x) state[,,Es[x]]))
  out[,,"I"] <- sum(sapply(infectious_shape, FUN=function(x) state[,,Is[x]]))
  out[,,"R1"] <- sum(sapply(immune_shape, FUN=function(x) state[,,RAs[x]]))
  out[,,"R2"] <- sum(sapply(immune_shape, FUN=function(x) state[,,RBs[x]]))
